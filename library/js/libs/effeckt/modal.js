...
;(function(window){
  ...
  EffecktModal.prototype.bindUIActions = function() {
    jQuery(document).on( Effeckt.buttonPressedEvent,
                    '.effeckt-modal-button',
                    jQuery.proxy(this.open, this)
                  );

    jQuery(document).on( Effeckt.buttonPressedEvent,
                    '.effeckt-modal-close, [data-effeckt-dismiss="modal"]',
                    jQuery.proxy(this.close, this)
                  );
  ...
  EffecktModal.prototype.open = function( e ) {
    ...
    this.$element = jQuery(target || '#effeckt-modal-wrap');
    ...
    if ( this.$element.hasClass( this.modalEffeckt ) || this.modalEffeckt === undefined ) {
      ...
      this.$element.on( Effeckt.transitionAnimationEndEvent, jQuery.proxy(function() {
  ...
  EffecktModal.prototype.addOverlay = function() {
    ...
    this.$overlay = jQuery('<div ' + atts.join(' ') + ' />')
